...
import { Import_Users_Job_Status_Enum } from '@app/generated/graphql'
...
export const Importing: React.FC = () => {
  ...
  const result = data?.import_users_job_by_pk?.result
  const status = data?.import_users_job_by_pk?.status

  useEffect(() => {
    if (status && status !== Import_Users_Job_Status_Enum.InProgress) {
      ...
      {!result?.total ? (
        ...
        <>
          <Typography mb={4} display="flex">
            {status === Import_Users_Job_Status_Enum.InProgress ? (
              <>
            ...
            {status === Import_Users_Job_Status_Enum.Succeeded ? (
              <>
            ...
            {status === Import_Users_Job_Status_Enum.Failed ? (
              <>
            ...
            {status === Import_Users_Job_Status_Enum.PartiallySucceeded ? (
              <>
