...
import {
  ...
  GetMainOrganisationDetailsForDeleteQuery,
  GetMainOrganisationDetailsForDeleteQueryVariables,
} from '@app/generated/graphql'
import { useScopedTranslation } from '@app/hooks/useScopedTranslation'
import { DELETE_ORG } from '@app/modules/organisation/queries/delete-org'

import { GET_MAIN_ORGANISATION_DETAILS_FOR_DELETE } from './queries'
...
const DeleteOrgModal = ({ onClose, open, org }: DeleteOrgModalProps) => {
  ...
  const [{ data: orgData, error: fetchOrgError }] = useQuery<
    ...
    GetMainOrganisationDetailsForDeleteQueryVariables
  >({
    query: GET_MAIN_ORGANISATION_DETAILS_FOR_DELETE,
    variables: {
      orgId: org.id,
    },
  ...
  const deleteOrgModalData = useMemo<OrgAggregateData | null>(() => {
    ...
    return {
      count: {
        ...
        affiliatedOrgs: org.affiliatedOrgs?.aggregate?.count as number,
      },
    }
  }, [orgData?.orgs])
