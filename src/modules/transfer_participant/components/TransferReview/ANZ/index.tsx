...
> = () => {
  ...
  const useGST = isAustraliaCountry(fromCourse?.residingCountry)
              ...
              <>
                ...
                {useGST ? (
                  <InfoPanel data-testid="fee-vat-type-panel">
                    <InfoRow
                      label={t('review-transfer.gst')}
                      value={_t('currency', {
                        amount: new Big((fees.customFee * 10) / 100)
                          .round(2)
                          .toNumber(),
                        currency: fromCourse.priceCurrency,
                      })}
                  ...
                  <InfoRow>
                    ...
                    <Typography fontWeight={600}>
                      {_t('currency', {
                        amount:
                          fees.customFee +
                          new Big((fees.customFee * (useGST ? 10 : 0)) / 100) // if useGST is false, then 0% is added as tax
                            .round(2)
                            .toNumber(),
                        currency: fromCourse.priceCurrency,
                      })}
