CREATE OR REPLACE VIEW "public"."upcoming_enrollments" AS 
   ...
   FROM profile AS profile
     JOIN course_participant AS cp ON cp.profile_id = profile.id
     JOIN course AS course ON course.id = cp.course_id
     JOIN organization AS org ON course.organization_id = org.id
     JOIN course_schedule AS schedule ON schedule.course_id = course.id
     JOIN course_certificate AS cc ON cc.profile_id = profile.id
     JOIN course_participant AS course_p ON course_p.certificate_id = cc.id
  WHERE schedule."end" > now() AND cc.status IN ('ACTIVE','ON_HOLD','EXPIRED_RECENTLY','EXPIRING_SOON')
   AND profile.archived IS FALSE
   AND (course_p.completed_evaluation IS TRUE OR cc.legacy_course_code IS NOT NULL);
