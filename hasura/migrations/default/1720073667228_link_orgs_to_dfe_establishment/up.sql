ALTER TABLE "public"."organization"
    ADD COLUMN "dfe_establishment_id" UUID;
...
SET dfe_establishment_id = (
    ...
    WHERE o.name = dfe.name
      ...
      AND (
          ...
          AND (CASE WHEN dfe.head_last_name IS NOT NULL THEN dfe.head_last_name = o.attributes->>'headSurname' ELSE TRUE END)
          AND (CASE WHEN dfe.head_job_title IS NOT NULL THEN dfe.head_job_title = o.attributes->>'settingName' ELSE TRUE END)
          AND (CASE WHEN dfe.ofsted_rating IS NOT NULL THEN dfe.ofsted_rating ILIKE o.attributes->>'ofstedRating' ELSE TRUE END)
      )
    LIMIT 1
)
WHERE o.name IN (SELECT name FROM dfe_establishment);
