CREATE SEQUENCE course_id_seq START 10000 INCREMENT 1;

ALTER TABLE course
    ADD COLUMN autoincremental_id integer NOT NULL UNIQUE DEFAULT nextval('course_id_seq');

ALTER TABLE course_evaluation_answers ADD COLUMN "course_id_new" integer NULL;
ALTER TABLE course_invites ADD COLUMN "course_id_new" integer NULL;
ALTER TABLE course_leader ADD COLUMN "course_id_new" integer NULL;
ALTER TABLE course_module ADD COLUMN "course_id_new" integer NULL;
ALTER TABLE course_participant ADD COLUMN "course_id_new" integer NULL;
ALTER TABLE course_schedule ADD COLUMN "course_id_new" integer NULL;

UPDATE course_evaluation_answers SET course_id_new = c.autoincremental_id FROM course c WHERE c.id = course_id;
UPDATE course_invites SET course_id_new = c.autoincremental_id FROM course c WHERE c.id = course_id;
UPDATE course_leader SET course_id_new = c.autoincremental_id FROM course c WHERE c.id = course_id;
UPDATE course_module SET course_id_new = c.autoincremental_id FROM course c WHERE c.id = course_id;
UPDATE course_participant SET course_id_new = c.autoincremental_id FROM course c WHERE c.id = course_id;
UPDATE course_schedule SET course_id_new = c.autoincremental_id FROM course c WHERE c.id = course_id;
...
ALTER TABLE course RENAME COLUMN autoincremental_id TO id;
