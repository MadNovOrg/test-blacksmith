...
allowedUsers.forEach(allowedUser => {
  const dataSet = [
    {
      name: `open BILD advanced trainer F2F reaccred as ${allowedUser}`,
      user: `${allowedUser}`,
    ...
    {
      name: `open BILD advanced trainer F2F non-reaccred non-conversion as ${allowedUser}`,
      user: `${allowedUser}`,
    ...
    {
      name: `open BILD advanced trainer F2F conversion as ${allowedUser}`,
      user: `${allowedUser}`,
    ...
    {
      name: `open BILD advanced trainer mixed conversion as ${allowedUser}`,
      user: `${allowedUser}`,
      smoke: allowedUser === 'admin' ? '@smoke' : '',
      course: (() => {
        const course = UNIQUE_COURSE()
    ...
    {
      name: `open BILD advanced trainer virtual conversion as ${allowedUser}`,
      user: `${allowedUser}`,
      smoke: allowedUser === 'ops' ? '@smoke' : '',
      course: (() => {
        const course = UNIQUE_COURSE()
