...
allowedUsers.forEach(allowedUser => {
  ...
  for (const data of dataSet) {
    test(`${data.name} ${data.smoke}`, async ({ browser }) => {
      ...
      await individualOrgPage.checkOrgDetailsTab(true)
      await individualOrgPage.checkDeleteOrgButton(data.orgName)
      await individualOrgPage.checkEditOrgButton(orgId)
    })
  }
})
