...
;(
  ...
  test(`it allows ${role} user to insert a waitlist`, async () => {
    ...
    test.expect(hasAllowedFields).toEqual(true)
    test.expect(hasForbiddenFields).toBe(false)
  })
})
;(['sales-admin', 'trainer', 'tt-ops', 'tt-admin'] as const).forEach(role => {
  test(`it allows role ${role} to select waitlist`, async () => {
    ...
    test.expect(waitlistQuery).toBeTruthy()
    test.expect(waitlistsQuery).toBeTruthy()
    test.expect(hasAllowedFields).toBe(true)
  })
})
;(['unverified', 'anonymous'] as const).forEach(role => {
  test(`doesn't allow role ${role} to select waitlist`, async () => {
    ...
    test.expect(waitlistQuery).toBeFalsy()
    test.expect(waitlistsQuery).toBeFalsy()
  })
})
